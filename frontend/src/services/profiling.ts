...
const profilingCache = new Map<string, ProfilingResult>();
...
export function detectProfiling(input: SystemInput): ProfilingResult {
  ...
  if (profilingCache.size >= MAX_CACHE_SIZE) {
    const first = profilingCache.keys().next().value;
    if (first !== undefined) profilingCache.delete(first);
  }
