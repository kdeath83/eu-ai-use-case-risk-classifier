...
const miCache = new Map<string, MaterialInfluenceResult>();
...
export function scoreMaterialInfluence(input: SystemInput): MaterialInfluenceResult {
  ...
  if (miCache.size >= MAX_CACHE_SIZE) {
    const first = miCache.keys().next().value;
    if (first !== undefined) miCache.delete(first);
  }
  ...
  const autonomyScores: Record<string, number> = { none: 0, low: 10, medium: 35, high: 70, full: 90 };
  ...
  const sensitiveInputs = dataInputs.some(di => /biometric|personal|health|financial|criminal|behavior|location/i.test(di));
  if (sensitiveInputs) {
    score += 10;
    rationaleParts.push('Sensitive data inputs detected: +10 points');
  }

  const bindingOutputs = outputs.some(o => /decision|approval|denial|score|rating|classification|determination/i.test(o));
  if (bindingOutputs) {
    score += 10;
  ...
  const result: MaterialInfluenceResult = { score, threshold, exceedsThreshold, answers, rationale: rationaleParts.join('. ') + `. Final score: ${score}/100.` };
